...
class TextPreprocessor:
    ...
    def fetch_from_url(self, url: str) -> str:
        ...
        if not url or not url.endswith(".txt"):
            raise ValueError("URL must end with .txt")
    ...
    def get_text_statistics(self, text: str) -> Dict:
        ...
        """
        chars = self.tokenize_chars(text)
        ...
        avg_sentence_length = sum(len(self.tokenize_words(s)) for s in sentences) / sentence_count if sentence_count > 0 else 0
